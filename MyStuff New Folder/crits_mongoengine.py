...
class CritsDocument(BaseDocument):
    ...
    def to_stix(self, items_to_convert=[], loaded=False, bin_fmt="raw"):
        ...
        # Import methods to set namespace and alias
        # *** These localizations should reference config parameters vs. changing core  ***
        # Import as "stix_"to avoid collisions with other idgen.py modules namespaces
        ...
        # Import as "cybox_"to avoid collisions with other idgen modeules namespaces
        # *** There are at least 4 idgen.py modules ***
        from cybox.utils import set_id_namespace as cybox_set_id_namespace
        from cybox.utils import Namespace as cybox_Namespace
        from cybox.core import Observable
        # Set the STIX namsespace and alias
        STIXNAMESPACE = {'http://intinc.specere.org' : 'intdevNS'}
        stix_set_id_namespace(STIXNAMESPACE)
        # Set the CybOx namespace and alias - Note Cybox idgen doesn't use python dictionaries]
        CYBOXNAMESPACE = cybox_Namespace('http://intinc.specere.org' , 'intdevNS')
        cybox_set_id_namespace(CYBOXNAMESPACE)
        ...
        stix_msg['stix_obj'] = STIXPackage(indicators=stix_msg['stix_indicators'],
                        ...
                        stix_header=header,
                        id_= "%s:%s-%s" % (get_id_namespace_alias(), "package", uuid.uuid4()))
        return stix_msg
